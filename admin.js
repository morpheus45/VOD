// --- Admin guard (v9) ---
const ADMIN_KEY="vod_admin_ok_v1";
function isAuthed(){
  try{ return localStorage.getItem(ADMIN_KEY)==="1"; }catch(e){ return false; }
}
(function(){
  const p = (location.pathname||"").toLowerCase();
  if(p.endsWith("/panel.html") || p.endsWith("panel.html")){
    if(!isAuthed()){
...
function renderTable(){
  ...
  arr.forEach((it,i)=>{
    ...
    tr.innerHTML=`<td>${i+1}</td>
      <td data-field="title">${it.title||""}</td>
      <td data-field="category">${it.category||""}</td>
      <td class="mono" data-field="url">${it.url||""}</td>
      <td class="mono" data-field="image">${it.image||""}</td>
    ...
    // Double-click to edit (no UI change)
    tr.querySelectorAll("[data-field]").forEach(td=>{
      td.ondblclick=()=>{
        const field=td.getAttribute("data-field");
        const current=(it[field]||"").toString();
        const label = field==="title"?"Titre":field==="category"?"Catégorie":field==="url"?"URL vidéo":"URL image";
        const nv = prompt("Modifier " + label + " :", current);
        if(nv===null) return;
        const value = (nv||"").trim();
        if(field==="url"){
          try{ new URL(value); }catch(e){ alert("URL vidéo invalide."); return; }
        }
        if(field==="image" && value){
          try{ new URL(value); }catch(e){ alert("URL image invalide."); return; }
        }
        entries[idx][field]=value;
        if(field==="category") ensureCat(value);
        save(); renderAll();
      };
    });
