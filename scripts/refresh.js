/**
 * scripts/refresh.js — PIPSILY v1.1 (FIX intelligent)
 * ─────────────────────────────────────────────────────
 * Rafraîchit TOUTES les données IPTV avec logique de comparaison :
 *   • series.json   — ajoute / met à jour / supprime les séries
 *   • vod.json      — liste complète des films
 *   • live.json     — chaînes TV
 *   • episodes_part*.json — merge intelligent (nouvelles saisons, épisodes manquants)
...
const PROGRESS_FILE  = path.join(ROOT, "refresh_progress.json");
...
function loadExistingEpisodes(){
  ...
  for(let i = 1; i <= 20; i++){
    ...
    try { Object.assign(db, JSON.parse(fs.readFileSync(f, "utf8"))); }
    catch(e) {}
  }
  return db;
}

// ─── Progress (reprise après coupure) ─────────────────────────────────────────
...
// ─── Construction données épisodes ────────────────────────────────────────────
function buildEpisodeData(data, creds){
  const seasons = {};
  const seasonsMeta = [];
...
// ─── Comparaison épisodes (deep check) ────────────────────────────────────────
function hasEpisodeChanges(local, remote){
  if(!local || !remote) return true;

  // Meta
  if(local.meta?.cover !== remote.meta?.cover) return true;
  if(local.meta?.plot  !== remote.meta?.plot)  return true;

  // Saisons
  const localMeta  = local.seasonsMeta  || [];
  const remoteMeta = remote.seasonsMeta || [];
  if(localMeta.length !== remoteMeta.length) return true;
  for(let i = 0; i < localMeta.length; i++){
    if(localMeta[i].num   !== remoteMeta[i].num)   return true;
    if(localMeta[i].count !== remoteMeta[i].count) return true;
    if(localMeta[i].cover !== remoteMeta[i].cover) return true;
    if(localMeta[i].name  !== remoteMeta[i].name)  return true;
  }

  // Épisodes par saison
  const localSeasons  = local.seasons  || {};
  const remoteSeasons = remote.seasons || {};
  const localKeys  = Object.keys(localSeasons).sort((a,b)=>Number(a)-Number(b));
  const remoteKeys = Object.keys(remoteSeasons).sort((a,b)=>Number(a)-Number(b));
  if(localKeys.length !== remoteKeys.length) return true;
  for(let i = 0; i < localKeys.length; i++){
    if(localKeys[i] !== remoteKeys[i]) return true;
    const lep = localSeasons[localKeys[i]];
    const rep = remoteSeasons[remoteKeys[i]];
    if(!Array.isArray(lep) || !Array.isArray(rep)) return true;
    if(lep.length !== rep.length) return true;
    for(let j = 0; j < lep.length; j++){
      if(lep[j].id    !== rep[j].id)    return true;
      if(lep[j].title !== rep[j].title) return true;
      if(lep[j].url   !== rep[j].url)   return true;
    }
  }
...
async function refreshSeries(creds){
  ...
  const existing = loadExistingSeries();
  ...
  items.forEach(item => {
    const sid = String(item.series_id);
    const old = existing[sid];
    if(!old){
      newIds.add(sid);
      changedIds.add(sid);
    } else {
      const metaChanged = (
        ...
        old.stream_icon   !== item.stream_icon   ||
        old.plot          !== item.plot          ||
        old.category_id   !== item.category_id   ||
        old.category_name !== item.category_name ||
        old.added         !== item.added         ||
        old.quality       !== item.quality
      );
      if(metaChanged) changedIds.add(sid);
    }
  });

  // Séries supprimées chez le fournisseur
  const currentIds = new Set(items.map(i => String(i.series_id)));
  const removedIds = Object.keys(existing).filter(sid => !currentIds.has(sid));
  ...
  console.log(`      ${newIds.size} nouvelles | ${changedIds.size} modifiées | ${removedIds.length} supprimées`);

  return { items, changedIds, newIds, removedIds };
}
...
// ─── Refresh épisodes (intelligent + incrémental) ────────────────────────────
async function refreshEpisodes(creds, seriesResult){
  console.log("\n4/4  Épisodes (intelligent)...");
  const { items: seriesItems, changedIds, newIds, removedIds } = seriesResult;

  const existingEpisodes = loadExistingEpisodes();
  const progress = loadProgress();

  // 1. Supprimer les séries orphelines (supprimées chez le fournisseur)
  let removedCount = 0;
  for(const sid of removedIds){
    if(existingEpisodes[sid]){
      delete existingEpisodes[sid];
      removedCount++;
    }
  }
  if(removedCount > 0){
    console.log(`   Orphelines supprimées : ${removedCount} séries`);
  }

  // 2. Déterminer les séries à vérifier
  //    On vérifie TOUTES les séries actuelles, sauf celles déjà traitées
  //    dans un run précédent qui s'est interrompu (progress file).
  const allCurrentIds = seriesItems.map(s => String(s.series_id || s.id));
  const idsToProcess  = allCurrentIds.filter(sid => !progress.done.has(sid));

  console.log(`   Cache local  : ${Object.keys(existingEpisodes).length} séries`);
  console.log(`   À vérifier   : ${idsToProcess.length} séries`);

  if(idsToProcess.length === 0){
    console.log("   ✓ Base épisodes déjà à jour");
    writeChunks(existingEpisodes);
    return;
  }
  ...
  async function processOne(sid){
    const s = seriesItems.find(x => String(x.series_id || x.id) === sid);
    if(!s) return;
    ...
    const remote = buildEpisodeData(data, creds);
    const local  = existingEpisodes[sid];

    if(local && !hasEpisodeChanges(local, remote)){
      unchanged++;
      return;
    }

    existingEpisodes[sid] = remote;
    updated++;
  }

  // 3. Traitement par batches avec sauvegarde de progression
  for(let i = 0; i < idsToProcess.length; i += CONCURRENCY){
    const batch = idsToProcess.slice(i, i + CONCURRENCY);
    await Promise.all(batch.map(processOne));
    fetched += batch.length;

    // Marquer comme traitées et sauvegarder la progression
    batch.forEach(sid => progress.done.add(sid));
    saveProgress(progress);

    if(fetched % 200 === 0 || fetched >= idsToProcess.length){
      process.stdout.write(`\r   ${fetched}/${idsToProcess.length} vérifiés — ${updated} mis à jour, ${unchanged} inchangés, ${failed} échoués`);
    }
  }

  console.log(`\n   ✓ ${updated} séries mises à jour, ${unchanged} inchangées, ${failed} échouées`);
...
async function main(){
  ...
  console.log("║  v1.1  (fix intelligent)           ║");
  console.log("╚══════════════════════════════════════╝");

  const creds = getCredentials();
  console.log(`\n📡  API : ${creds.base}  (user: ${creds.username})`);

  const seriesResult = await refreshSeries(creds);
  if(!seriesResult){ process.exit(1); }

  await refreshVod(creds);
  await refreshLive(creds);
  await refreshEpisodes(creds, seriesResult);
