...
/**
 * PIPSILY — Mini proxy HTTP local qui relaie vers le SOCKS5 de Tor.
 *
 * Pourquoi : Tor n'expose qu'un proxy SOCKS5 (127.0.0.1:socksPort). On expose ici
 * un proxy HTTP local (127.0.0.1:<port aléatoire>) que l'OkHttpClient d'ExoPlayer
 * utilise comme Proxy.Type.HTTP → chaque flux sport traverse Tor.
 *
 * Formes gérées :
 *   - CONNECT host:port           (tunnel, ex. HTTPS)
 *   - GET http://host/path ...    (forme absolue, ex. HTTP clair goldenlink.live)
 *
 * IMPORTANT : la résolution DNS est déléguée à Tor (ATYP=domaine dans le SOCKS5),
 * indispensable si le FAI bloque aussi la résolution du domaine.
 *
...
final class HttpToSocksProxy {
    ...
    HttpToSocksProxy(int socksPort) {
        this.socksPort = socksPort;
    }

    /** Démarre l'écoute sur un port local libre et renvoie ce port. */
    int start() throws IOException {
        ...
        accept.start();
        Log.i(TAG, "HTTP->SOCKS proxy sur 127.0.0.1:" + port + " (socks=" + socksPort + ")");
        return port;
    }
    ...
    // ── Traitement d'une connexion cliente ─────────────────────────────
    ...
    private void handle(Socket client) {
        ...
        try {
            ...
            String requestLine = headText.substring(0, eol);
            ...
            if ("CONNECT".equalsIgnoreCase(method)) {
                ...
                String newRequestLine = method + " " + path + " " + (parts.length >= 3 ? parts[2] : "HTTP/1.1");
                rewrittenHeader = newRequestLine + headText.substring(eol);
            }
            ...
            if (connect) {
                writeLine(client, "HTTP/1.1 200 Connection established\r\n\r\n");
            } else {
                rout.write(rewrittenHeader.getBytes(StandardCharsets.ISO_8859_1));
                rout.flush();
            }
    ...
    // ── SOCKS5 (RFC 1928) : handshake no-auth + CONNECT par nom de domaine ──
