...
public class PlayerActivity extends FragmentActivity {
    ...
    // [VPN test] Client OkHttp routé via Tor (proxy HTTP local) — chaînes sport bloquées
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        torHttpPort     = getIntent().getIntExtra("torHttpPort", -1); // [VPN test] >0 → via Tor
    ...
    private OkHttpDataSource.Factory buildDsFactory() {
        OkHttpClient client;
        if (torHttpPort > 0) {
            // [VPN test] Chaîne sport bloquée par le FAI → CE flux passe par Tor.
            if (torClient == null) {
